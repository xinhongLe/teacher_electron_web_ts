<template>
    <div class="question-box">
        ...
        <div
            v-if="questionType == 7"
            class="speech"
            :class="isQuestion ? 'basic' : isOrigin ? 'basic' : ''"
        >
            <!-- <img class="title-img" src="./images/read.png" alt /> -->
            <div v-html="text"></div>
        </div>
        <div v-if="questionType == 7" class="speech-audio-box">
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const dealSpeechText = () => {
            ...
            speechResult.map((item, index) => {
                const result = JSON.parse(
                    JSON.parse(item.JsonData).Result
                ).result;

                audioList.value.push({
                    time: dealTime(result.wavetime),
        ...
        const playAudio = (i: number) => {
            ...
            playIndex.value = i;
            speechAudio.value = props.speechAudioList[i];
