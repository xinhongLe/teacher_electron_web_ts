...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const subjectList = computed(() => {
            return allBookList.value.find(item => item.Id === schoolSection.value)?.Children || [];
        });

        // 版本列表
        const versionList = computed(() => {
            return subjectList.value.find(item => item.Id === subject.value)?.Children || [];
        });

        // 年级列表
        const gradeList = computed(() => {
            return versionList.value.find(item => item.Id === version.value)?.Children || [];
        });

        // 直接给变量赋值不触发change事件 所以采用watch进行监听
        // 学段变化
        watch(schoolSection, async () => {
            subject.value = subjectList.value.length > 0 ? subjectList.value[0].Id : "";
        });

        // 因为数据存在科目和版本id不变的情况 下面监听多增加上一层变化的监听！！！！！！！！！！！
        // 科目变化
        watch([subject, schoolSection], async () => {
            version.value = versionList.value.length > 0 ? versionList.value[0].Id : "";
        });

        // 版本变化
        watch([version, subject, schoolSection], async () => {
            grade.value = gradeList.value.length > 0 ? gradeList.value[0].BookId || "" : "";
        });
        ...
        const selectedBookName = computed(() => {
            return selectedBook.value.SubjectName + " " + selectedBook.value.PublisherName + " " + selectedBook.value.AlbumName;
        });
