...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const init = () => {
            form.book = [
                props.directory.schoolSection.id,
                props.directory.subject.id,
                props.directory.version.id,
                props.directory.grade.id,
            ];
            form.chapterAndLesson = [
                props.directory.chapter.id,
                props.directory.lesson.id,
            ];
        ...
        const getBook = () => {
            const subjectPublisherBookList =
                store.state.preparation.subjectPublisherBookList;
            const A = subjectPublisherBookList.find(
                (item) => item.Id === form.book[0]
            );
        ...
        const getChapter = () => {
            const chapter = storeChapterList.find(
                (item) => item.Id === form.chapterAndLesson[0]
            );
            const lesson =
