<template>
    <div class="set-content">
        <el-form ref="formRef" :model="form" :rules="rules" label-width="70px" label-position="right">
            <div class="header">
                <el-form-item label="类型：">
                    <el-select @change="changeType" v-model="form.Type" class="m-2" placeholder="Select" size="large">
                        <el-option
                ...
                <el-form-item label="背景：">
                    <el-select v-model="form.ThemeId" class="m-2" placeholder="Select" size="large">
                        <el-option
            ...
            <div class="add-box">
                ...
                <span>添加分类</span>
                <span>( <span class="num">{{form.classData.length}}</span>/5 )</span>
            </div>
            <div class="content-box">
                <div v-for="(item, index) in form.classData" :key="index" class="row">
                    <div>
                        <div class="text">
                            <img v-if="index > 0" @click="delQuestion(index)" src="@/assets/projection/icon_delete.png" alt="">
                            分类{{index + 1}}：
                        </div>
                        <div v-if="form.Type === 1" class="option-col">
                            <el-form-item label-width="0px" :prop="`classData[${index}].Data`" :rules="rules.Data">
                                <el-input v-model="item.Data"  :autosize="{ minRows: 4, maxRows: 4 }" placeholder="请输入分类名称" type="textarea"/>
                            </el-form-item>
                        </div>
                        <div v-else class="option-col">
                            <el-form-item label-width="0px" :prop="`classData[${index}].File.url`" :rules="rules.File">
                                <el-upload
                                    ref="uploadRef"
                                ...
                                >
                                    ...
                                    <el-icon v-else class="avatar-uploader-icon"><Plus />上传图片</el-icon>
                                </el-upload>
                            </el-form-item>
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="row" style="flex-wrap: wrap;">
                            <div v-for="(option, i) in item.Item" :key="i" class="option-col">
                                <div v-if="form.Type === 3">
                                    <img v-if="i > 0" class="del_img" @click="delOption(index, i)" src="@/assets/projection/icon_delete.png" alt="">
                                    <el-form-item label-width="0px"  :prop="`classData[${index}].Item[${i}].File.url`" :rules=" rules.File">
                                        <el-upload
                                            class="avatar-uploader"
                                            action=""
                                            accept=".jpg,.jpeg,.gif,.png"
                                            :auto-upload="false"
                                            :on-change="(file) => onChange(file, index, i)"
                                            :show-file-list="false"
                                        >
                                            <img v-if="option.File.url" :src="option.File.url" class="avatar" />
                                            <el-icon v-else class="avatar-uploader-icon"><Plus />上传图片</el-icon>
                                        </el-upload>
                                    </el-form-item>
                                </div>
                                <div v-else>
                                    <img v-if="i > 0" class="del_img" @click="delOption(index, i)" src="@/assets/projection/icon_delete.png" alt="">
                                    <el-form-item label-width="0px" :prop="`classData[${index}].Item[${i}].Data`" :rules="rules.Data">
                                        <el-input v-model="option.Data"  :autosize="{ minRows: 4, maxRows: 4 }" placeholder="请输入分类选项" type="textarea"/>
                                    </el-form-item>
                                </div>
                            </div>
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const upload = async (file:any, imgWidth:number, imgHeight:number, index:number, i: undefined | number) => {
            ...
            if (file) {
                ...
                reader.onload = function (event:any) {
                    ...
                    img.onload = function () { // 注意只有onload以后才可以拿到图片信息
                        ...
                        if (imgWidth > 540 || imgHeight > 960 || (file.raw.size / 1024) > 1024) {
                            return ElMessage.warning("图片像素不超过540 * 960，且文件大小不超过1M");
                        }
        ...
        const addQuestion = () => {
            ...
            form.value.classData.push({
                ...
                Type: form.value.Type === 1 ? 1 : 0,
                File: { url: "", Bucket: "", Name: "", FileName: "", FilePath: "", Extention: "", Type: 2 },
                Position: { x: 260 * (form.value.classData.length + 1), y: 600 },
                Size: { Width: 297, Height: 377 },
                Item: [
                    {
                        Data: "",
                        Type: form.value.Type === 3 ? 0 : 1,
                        File: { url: "", Bucket: "", Name: "", FileName: "", FilePath: "", Extention: "", Type: 2 },
                        Position: { x: 200, y: 168 * (form.value.classData.length + 1) },
                        Size: { Width: 240, Height: 168 }
                    }
                ]
            });
            ...
            form.value.classData[index].Item.push(
                {
                    ...
                    Type: form.value.Type === 3 ? 0 : 1,
                    File: { url: "", Bucket: "", Name: "", FileName: "", FilePath: "", Extention: "", Type: 2 },
                    Position: { x: 200 * (form.value.classData[index].Item.length + 1), y: 168 * (index + 1) },
                    Size: { Width: 240, Height: 168 }
                }
            );
        };
