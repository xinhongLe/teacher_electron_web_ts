...
import { ClassContent } from "@/hooks/useBag";
import { store } from "@/store";
import { AddCourseWareTeacherElementFileData, BagPapers, Material, SchoolBagInfo, UpdateCourseWareListOfTeacherData } from "@/types/preparation";
...
export default (callback: () => Promise<void>, classContentList: Ref<ClassContent[]>, selectCourseBag: Ref<SchoolBagInfo | undefined>) => {
    let toSortI = -1;
    ...
    const onDrop = async (e: DragEvent, index: number, data?: CourseWares) => {
        ...
        if (dragInfo.isElement) { // 拖拽精品素材
            ...
            const courseBagTeacherID = selectCourseBag.value?.ID || "";
        ...
        if (dragInfo.ID) {
            ...
            const data: UpdateCourseWareListOfTeacherData = {
                addedCourseWareTeacher: [{
                    CourseBagTeacherID: selectCourseBag.value?.ID || "",
                    type: 1,
                    process: index + 1,
                    ...info,
