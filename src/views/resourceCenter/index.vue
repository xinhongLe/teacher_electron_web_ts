...
import {defineComponent, onActivated, onDeactivated, computed} from "vue";
...
export default defineComponent({
    ...
    setup() {
        ...
        // webview地址
        const url = computed(() => {
            return `${RESOURCE_WEB}/#/resource?TOKEN=${token}&USERID=${userId.value}&ORGID=${orgID}&SECRETKEY=${secretKey}&USERTYPE=${userType}&SCHOOLID=${schoolId.value}&SCHOOLNAME=${schoolName.value}&PLATTYPE=${platType}&platformId=${platformId}`
        });
        // console.log('url----', url);
        return {
            url,
