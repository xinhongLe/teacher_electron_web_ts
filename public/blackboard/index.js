...
let origin = { x: 0, y: 0 };
let isMoving = false;
let Broadnumber = gn("openBroadList");
...
openBroadList.onclick = function() {
  let dataUrl = fabCanvas.toJSON();
  let canvasimg = fabCanvas.toDataURL();
  let data = { data: dataUrl, img: canvasimg };
  storageCanvasData[boardIndex - 1] = data;
  console.log("storageCanvasData", storageCanvasData);
  let canvasbroads = document.querySelectorAll(".canvas_broad");
...
function drawboradImage() {
  let broad = gn(`broad_${boardIndex}`);
  let chldEle = broad.children;
  var image = new Image();
  image.src = fabCanvas.toDataURL();
  let imgSrc = fabCanvas.toDataURL();
  if (chldEle[2]) {
...
gn("imgList").addEventListener(
  ...
  function(event) {
    ...
    if (event.target.className == "canvas_broad_del") {
      // 删除事件
      let deElement = event.target.parentNode;
      let delDomId = deElement.id;
      let ind = delDomId.charAt(delDomId.length - 1);
      boardIndex--;
      Broadnumber.innerHTML = boardIndex;
      let divEle = gn("imgList");
    ...
    let checkedDomId = parentTarget.id;
...
function creatBroadlist() {
  ...
  for (let i = 1; i < storageCanvasData.length + 1; i++) {
    let broadhtml = `<div id="broad_${i}" class="canvas_broad">
